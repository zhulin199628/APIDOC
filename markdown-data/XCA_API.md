...
XCA是Axalent Cloud设备端的程序连接库，通常运行在资源有限的设备平台，接入单个设备节点，例如WIFI、GPRS、NB等可联网设备。占用资源少，可为多种不同平台编译程序链接库。  
...
调用此函数成功之后，XCA会使用不加密的方式将设备登录到服务器，并且维持设备和服务器的连接。若设备异常掉线，XCA会自动将设备登录重新登录到服务器。
...
调用此函数成功之后，XCA会使用可选(加密/不加密)的方式将设备登录到服务器，并且维持设备和服务器的连接。若设备异常掉线，XCA会自动将设备登录重新登录到服务器。
